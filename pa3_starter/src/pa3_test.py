...
class TestRoutes(unittest.TestCase):
    ...
    def test_create_user(self):
        ...
        self.assertEqual(
            ...
            list,
            wrong_value_error(
                req_type, route, type(user.get("transactions")), list, "type of transactions field"
            ),
        )
    ...
    def test_send_money(self):
        ...
        res = requests.post(gen_transactions_path(), data=json.dumps(transaction_body))
        ...
        self.assertEqual(
            ...
            str,
            wrong_value_error(
                req_type, route, type(transaction.get("timestamp")), str, "type of timestamp field"
            ),
        )
        ...
        self.assertEqual(
            ...
            1,
            wrong_value_error(
                req_type, route, len(res1.get("transactions")), 1, "length of transactions"
            )
        )
        for t in res1.get("transactions"):
            ...
            self.assertEqual(
                ...
                str,
                wrong_value_error(
                    req_type, route, type(t.get("timestamp")), str, "type of timestamp field"
                ),
            )
        ...
        self.assertEqual(
            ...
            1,
            wrong_value_error(
                req_type, route, len(res2.get("transactions")), 1, "length of transactions"
            )
        )
    ...
    def test_accept_request(self):
        ...
        create_res = requests.post(gen_transactions_path(), data=json.dumps(transaction_body))
        ...
        res = requests.post(gen_transactions_path(tr), data=json.dumps({"accepted":True}))
    ...
    def test_deny_request(self):
        ...
        res = requests.post(gen_transactions_path(tr), data=json.dumps({"accepted": False}))
        self.jsonable_test(res, req_type, route, 200, transaction_body)
        transaction = res.json()
        for key in transaction_body.keys():
            self.assertEqual(
    ...
    def test_get_user_transactions(self):
        ...
        create_res1 = requests.post(gen_transactions_path(), data=json.dumps(transaction_body))
        self.assertEqual(create_res1.status_code, 201)

        transaction_body2 = gen_transaction_body(user2, user1, True)
        create_res2 = requests.post(gen_transactions_path(), data=json.dumps(transaction_body2))
        self.assertEqual(create_res2.status_code, 201)

        transaction_body3 = gen_transaction_body(user1, user2, None)
        create_res3 = requests.post(gen_transactions_path(), data=json.dumps(transaction_body3))
        self.assertEqual(create_res3.status_code, 201)
        create_res3 = requests.post(gen_transactions_path(create_res3.json()["id"]), data=json.dumps({"accepted": False})).json()
        ...
        self.assertEqual(
            ...
            3,
            wrong_value_error(
                req_type, route, len(res.get("transactions")), 3, "length of transactions"
            )
        )
        ...
        for te, tr in zip(txns, expected):
            for key in te.keys():
                self.assertEqual(
                    tr.get(key),
                    te.get(key),
                    wrong_value_error(req_type, route, tr.get(key), te.get(key), te.get(key))
                )
                self.assertEqual(
                    type(tr.get("timestamp")),
                    str, 
                    wrong_value_error(req_type, route, type(tr.get("timestamp")), str, "type of timestamp field")
                )
    ...
    def test_extra_create_friends(self):
        if not EXTRA_CREDIT:
            return
        user1 = requests.post(gen_users_path(), data=json.dumps(SAMPLE_USER)).json().get("id")
        user2 = requests.post(gen_users_path(), data=json.dumps(SAMPLE_USER)).json().get("id")
        ...
        res2 = requests.get(gen_users_path(user1, True) + "friends/").json().get("friends")
        for f in res2:
            self.assertEqual(
                f.get("id"), 
                user2
            )
        self.assertEqual(len(res2), 1, wrong_value_error(req_type, route, len(res2), 1, "length of friends"))
        res3 = requests.get(gen_users_path(user2, True) + "friends/").json().get("friends")
        for f in res3:
            self.assertEqual(
                f.get("id"), 
                user1
            )
        self.assertEqual(len(res3), 1, wrong_value_error(req_type, route, len(res3), 1, "length of friends"))
    ...
    def test_extra_get_friends(self):
        ...
        self.jsonable_test(res, req_type, route, 200)
        res = res.json()
        self.assertEqual(type(res.get("friends")), list, wrong_value_error(req_type, route, type(res.get("friends")), list, "type of friends field"))
    ...
    def test_join(self):
        ...
        txn1_response = requests.post(gen_transactions_path(), data=json.dumps(txn1)).json()
        txn2 = gen_transaction_body(user1, user2, None)
        txn2_response = requests.post(gen_transactions_path(), data=json.dumps(txn2)).json()
        txn3 = gen_transaction_body(user1, user2, None)
        txn3_response = requests.post(gen_transactions_path(), data=json.dumps(txn3)).json()
        requests.post(gen_transactions_path(txn3_response["id"]), data=json.dumps({"accepted": False}))
        txn3["accepted"] = False
        expected = [{
        ...
        self.assertEqual(len(res), 3, wrong_value_error(req_type, route, len(res), 3, "length of result"))
