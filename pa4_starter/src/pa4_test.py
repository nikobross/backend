...
class TestRoutes(unittest.TestCase):
    ...
    def test_add_instructor_to_course(self):
        ...
        self.assertEqual(
            ...
            list,
            wrong_value_error(
                req_type, route, type(course.get("students")), list, "type of instructors field, test2"
            ),
        )
        self.assertEqual(
        ...
        req_type = "GET"
        route1 = gen_users_path(user_id)
        res = requests.get(route1)
        self.jsonable_test(res, req_type, route, 200, SAMPLE_USER)
        user = res.json()
        self.assertEqual(
            len(user.get("courses")),
        ...
        for course in user.get("courses"):
            ...
            self.assertEqual(
                ...
                None,
                wrong_value_error(
                    req_type, route1, course.get("instructors"), None, f"type of instructors field test3 {course}"
                ),
            )
